        ...
        if print_response:
            ...
            async for chunk in stream:
                ...
                if (hasattr(chunk, 'message') and hasattr(chunk.message, 'tool_calls') and
                    ...
                    print(f"[DEBUG] Found tool calls in chunk: {len(chunk.message.tool_calls)}")
                    for tool in chunk.message.tool_calls:
                        print(f"[DEBUG] Tool call: {tool}")
                        tool_calls.append(tool)
                else:
                    # Check if content contains JSON-like tool calls
                    if (hasattr(chunk, 'message') and hasattr(chunk.message, 'content') and
                        chunk.message.content and 'hexstrike-ai.' in chunk.message.content):
                        print(f"[DEBUG] Possible JSON tool call in content: {chunk.message.content[:100]}")
            ...
            async for chunk in stream:
                ...
                if (hasattr(chunk, 'message') and hasattr(chunk.message, 'tool_calls') and
                    chunk.message.tool_calls):
                    print(f"[DEBUG Non-print] Found tool calls: {len(chunk.message.tool_calls)}")
                    for tool in chunk.message.tool_calls:
                        print(f"[DEBUG Non-print] Tool: {tool}")
                        tool_calls.append(tool)

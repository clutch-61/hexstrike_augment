...
```bash
cd /disk1/users/renbh/ollama_mcp
...
默认路径为 `/disk1/users/renbh/UltraRAG`。若你的仓库位于其它位置，请在 shell 中设置：
...
```json
{
  "mcpServers": {
    "ultrarag-knowledge-base": {
      "command": "python3",
      "args": ["/disk1/users/renbh/ollama_mcp/ultrarag_mcp_server.py"],
      "env": {
        "PYTHONPATH": "/disk1/users/renbh/ollama_mcp",
        "ULTRARAG_ROOT": "/disk1/users/renbh/UltraRAG"
      }
    }
  }
}
...
```bash
ollmcp --servers-json /disk1/users/renbh/ollama_mcp/ultrarag_mcp_config.json
```
